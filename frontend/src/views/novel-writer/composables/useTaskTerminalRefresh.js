 ...
 *
 * 约束：
 * - 每个任务只校准一次（refreshedTaskId 防重）
 * - 任一来源确认终态后立即关闭实时连接，取消可能尚未触发的重连
 * - 仅"运行中 → 终态"迁移触发；初次加载即为终态的任务不重复刷新
...
export function useTaskTerminalRefresh(writingStore) {
  /** 已完成终态校准的任务ID（每任务只刷新一次） */
  let refreshedTaskId = null
  ...
  watch(
    ...
    async (status, prevStatus) => {
      ...
      if (!taskId || refreshedTaskId === taskId) return
