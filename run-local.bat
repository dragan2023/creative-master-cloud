...
for /f "tokens=5" %%a in ('netstat -ano ^| findstr ":7000.*LISTENING" 2^>nul') do (
    echo [清理] 终止后端进程 PID: %%a
    taskkill /F /PID %%a >nul 2>&1
    set BACKEND_CLEANED=1
)
if "%BACKEND_CLEANED%"=="1" (
    echo [清理] 后端端口 7000 已释放
...
REM 确保使用正确的环境变量启动 Vite
start "Creative-Master-Frontend" cmd /c "set BROWSER=none && npm run dev"
cd /d "%PROJECT_ROOT%"
