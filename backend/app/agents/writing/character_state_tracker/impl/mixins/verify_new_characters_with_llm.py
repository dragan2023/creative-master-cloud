...
class VerifyNewCharactersWithLlmMixin:
    ...
    async def verify_new_characters_with_llm(
        ...
        try:
            ...
            prompt = f"""你是一个严格的角色名称验证器。请逐项判断以下候选名称是否真的是小说中的人物角色名称。

⚠️ 重要区分标准：
- ✅ 人物角色名称：真实的姓名/称呼，如"张三"、"李小姐"、"王将军"、"林雪儿"
- ❌ 非人物名称（必须排除）：
  * 日常寒暄用语：恭喜、多谢、告辞、参见、拜见
  * 动作描述：抬起头、转过身、走上前
  * 语法片段：您把、臣把、不负、会各
  * 称谓组合（无具体姓氏）：教土司、老将军、大将军
  * 任何看起来不像真实姓名的短语

候选名称列表及其上下文：
{chr(10).join(char_contexts)}

请返回一个JSON数组，只包含确认是真实人物角色的名称。如果全部都不是人物，返回空数组[]。
例如：["张三", "李四"]
