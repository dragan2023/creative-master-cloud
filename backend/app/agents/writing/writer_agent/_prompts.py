...
class WriterPromptsMixin:
    ...
    def _build_direct_writer_system_prompt(self, context: AgentContext) -> str:
        ...
        if content_type in ("series_script", "movie_script"):
            ...
            base_prompt += "- **秒级时间轴**：每个镜头标注精确到秒的时间轴（[0-3秒]、[3-7秒]…），15秒镜头推荐四段式（0-3秒Hook → 3-7秒揭示 → 7-11秒细节/动作 → 11-15秒定格），分镜总时长应与剧本正文预估时长保持协调\n"
            base_prompt += "- **情感意图**：每个镜头想传达的情感或叙事目的\n"
            base_prompt += "- **转场方式**：切/淡入淡出/叠化/闪回等\n"
            base_prompt += "- **整段式输出**：虚拟模式下每个分镜必须输出为一段完整中文自然语句（禁止拆成字段列表），按时间轴依次描述运镜、场景变化与主体人物动作\n"
            base_prompt += "- **素材提示词**：涉及AI视频生成的，须配套输出对应图片素材（人物/场景/物品参考图）与音频素材（环境音/拟音/人声/BGM）的提示词或具体描述，适配 Seedance 2.0 / MiniMax H3 多模态参考\n"
        ...
        if content_type in ("series_script", "movie_script"):
            ...
            base_prompt += "4. **配乐智能判断（虚拟模式必做）**：AI自行判断每个关键场景是否需要背景音乐——" \
                "情绪强烈、需要氛围烘托的场景配BGM；纯对白或纪实段落可不配BGM或仅用环境音。若配BGM，须明确" \
                "起始点（第X镜头、第几秒进入/淡出），并判断用纯音乐（instrumental / no vocals）还是有人声歌唱的" \
                "（vocal，如 female vocalist / male vocal / choir / 哼唱 humming），两类 Suno 均可生成。\n"
