...
2. **为状态发生变化的设施生成状态实体**：
   ...
   {{
     ...
     "description": "在本章中，锦绣坊因经营不善而暂停营业"
   }}
   ```

3. **如果提示中给出了已知设施列表，检查本章内容是否涉及这些设施的状态变化。**
...
| 事件状态变化 | 事件阶段变化 | 事件名称, 当前阶段 | 当前阶段="已完成" |
| 事件影响 | 事件产生的后果 | 事件名称, 受影响实体, 影响类型 | 悬壶堂声誉受损 |
...
1. **识别已完成事件**：仔细阅读本章内容，找出所有在本章中**得到解决/完结/收尾**的事件。
   判定标准：
   - 冲突被化解（如案件破获、争端和解、战斗结束）
   - 目标达成（如任务完成、物品找到、人物获救）
   - 悬念揭晓（如真相大白、谜团解开）
   - 阶段性结束（如旅程到达目的地、仪式完成）

2. **为每个已完成事件生成"事件状态变化"实体**：
   ```json
   {{
     "text": "事件名称 - 已完成",
     "type": "事件状态变化",
     ...
     "description": "在本章中，悬壶堂中毒案的真凶被抓获，案件正式告破"
   }}
   ```

3. **为仍在进行中的事件生成状态更新**：如果某个已知事件在本章中推进了但没有完结，
   也要生成"事件状态变化"实体，`attributes.当前阶段` 设为当前具体阶段（如"深入调查中"、"对峙阶段"等）。

4. **检查已知未完成事件列表**：如果提示中给出了"未完成事件"上下文，
   必须逐一检查本章内容是否涉及这些事件的推进或完结。
...
2. **为状态发生变化的群体生成状态实体**：
   ...
   {{
     ...
     "description": "在本章中，娄山众吸纳了新成员，队伍扩大到五百人"
   }}
   ```

3. **如果提示中给出了已知群体列表，检查本章内容是否涉及这些群体的状态变化。**
...
1. **识别道具状态变化**：仔细阅读本章内容，找出道具的状态变化：
   - 道具被使用/消耗/销毁/遗失/损坏
   - 道具归属权转移（赠送、丢失、抢夺、继承）
   - 道具功能发生变化（升级、损坏、修复）
   - **判定标准**：道具在本章中是否被使用到"不再可用"或"转移给他人"
...
2. **为状态发生变化的道具生成状态实体**：
   ...
   {{
     ...
     "description": "在本章中，孙昭龙的手术刀因多次使用而磨损变钝"
   }}
   ```
   或归属变更：
   ```json
   {{
     ...
     "description": "孙昭龙将手术刀赠送给李大夫"
   }}
   ```

3. **如果提示中给出了已知道具列表，检查本章内容是否涉及这些道具的状态变化。**
...
- [ ] 关系数量不超过{max_relations}个
- [ ] ⚠️ 已检查本章中是否有事件完结：如有，已生成对应"事件状态变化"实体（当前阶段="已完成"）
- [ ] ⚠️ 已检查本章中是否有道具被使用/销毁/易主：如有，已生成"道具状态变化"或"道具归属变更"实体
- [ ] ⚠️ 已检查本章中是否有设施状态变化：如有，已生成"设施状态变化"实体
- [ ] ⚠️ 已检查本章中是否有群体状态变化：如有，已生成"群体状态变化"实体
- [ ] ⚠️ 已检查本章中是否有伏笔被回收：如有，已生成"伏笔回收"实体
