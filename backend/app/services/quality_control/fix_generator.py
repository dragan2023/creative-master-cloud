...
3. **分镜脚本设计**：场景切换规划、镜头语言(景别/运动方式/角度)、时长分配、情感意图、转场方式等（如存在）
4. **配乐参考**：音乐风格、情感基调、配乐建议等（如存在）
5. **其他附属部分**：标注、注释、格式标记等

**⚠️ 注意**: AI视觉资源(人物/场景/物品参考图提示词、Seedance 2.0视频提示词)已独立管理，不在正文内容中。正文修正后，用户可手动触发AI资源的重新生成。

**⚠️ 兼容性注意**: 如果原始内容中仍内嵌有 `### AI视觉资源生成`、`## 人物参考图生成提示词` 等旧格式AI资源段落（v2.6及之前版本），请**原样完整保留**，不要删除或修改。AI资源独立管理仅适用于新格式内容。

**严禁行为**：
- ❌ 严禁删除或截断「拍摄脚本参考」和「分镜脚本设计」部分
- ❌ 严禁在 fixed_content 中只返回正文而丢弃后续章节
- ❌ 严禁将分镜内容简化为"（分镜部分保持不变）"等占位符

**正确做法**：
- ✅ 只对正文中需要修正的部分进行修改
- ✅ 如果问题不涉及拍摄脚本或分镜设计部分，请原样完整保留
- ✅ 如果分镜设计部分需要根据正文修改进行调整，请在保留原有结构的基础上微调
- ✅ 修正后 fixed_content 的总长度不应显著少于原始内容

**视觉同步要求**（当正文修改涉及以下内容时必须同步更新）：
- ✅ 如果正文中角色外貌、服装、表情描述有变化 → 同步更新分镜设计中的相关画面描述
- ✅ 如果正文中场景环境、时间、天气、氛围有变化 → 同步更新分镜设计中的场景描述和镜头语言
- ✅ 如果正文剧情/动作序列有变化 → 同步更新分镜设计表中的画面描述、情感意图、时长分配
- ✅ 如果正文情绪基调有变化 → 同步更新配乐参考中的情感基调和音乐风格建议
- ✅ 确保分镜设计中引用的角色名、场景名与修正后正文完全一致
...
5. **保持风格**: 维持原有的文风和叙事风格
6. **尊重原文**: 只做必要的修改,不要重写整个内容
7. **分镜脚本设计同步**：如果正文修改影响了角色外貌、场景环境、动作设计等，必须同步更新分镜脚本设计中的画面描述、镜头语言、情感意图和时长分配
...
7. **保持风格**: 维持原有的文风和叙事风格
8. **尊重原文**: 只做必要的修改,不要重写整个内容
9. **分镜脚本设计同步**：如果正文修改影响了角色外貌、场景环境、动作设计等，必须同步更新分镜脚本设计中的画面描述、镜头语言、情感意图和时长分配
...
VISUAL_SYNC_PROMPT = """你是专业的剧本分镜设计同步编辑。你的任务是根据已修正的剧本正文，智能同步更新后续的分镜脚本设计、拍摄脚本参考和配乐参考。

【原始正文】（修正前，仅供参考了解修改了什么）
{original_content}

【修正后完整内容】（正文已被质控修正，分镜设计等部分需要同步更新）
{fixed_content}

## 核心任务

请仔细对比原始正文和修正后正文之间的差异，然后**只更新**分镜脚本设计和相关视觉指导部分，使它们与修正后的正文保持一致。**严禁修改正文部分**。

**⚠️ 注意**: AI视觉资源(人物/场景/物品参考图提示词、Seedance 2.0视频提示词)已独立管理，不在本同步范围内。

## 同步检查清单

### 1. 角色外貌/设定变化 → 更新分镜设计中的画面描述
如果正文中以下内容有变化：
- 角色的外貌描述（发型、服饰、年龄特征等）
- 角色的情绪状态或表情
- 角色的身份或定位
→ 同步更新分镜设计表中相关角色的画面描述和镜头语言

### 2. 场景环境变化 → 更新分镜设计中的场景描述
如果正文中以下内容有变化：
- 场景地点、环境描述
- 时间（日/夜）、天气、季节
- 场景氛围、色调
→ 同步更新分镜设计表中的场景描述、视觉风格、镜头类型

### 3. 剧情/动作变化 → 更新分镜设计和拍摄脚本
如果正文中以下内容有变化：
- 关键场景的动作序列
- 角色之间的互动方式
- 场景的戏剧张力和情感走向
...
### 4. 情绪基调变化 → 更新配乐参考
如果正文的情绪基调有变化：
→ 同步更新配乐参考中的情感基调和音乐风格建议

## 重要约束

- **严禁修改正文**：只更新分镜设计和视觉指导部分，正文部分一个字都不要改
- **保持格式**：所有分镜设计和视觉指导部分的格式、结构、字段顺序、Markdown标题层级完全不变
- **保持中文**：所有内容保持中文输出
- **完整输出**：返回完整内容（正文 + 所有分镜设计/视觉指导部分），不要省略任何内容
- **精确同步**：只更新确实受到正文修改影响的内容，未受影响的部分保持原样
- **一致性命名**：确保分镜设计中引用的角色名、场景名与修正后正文完全一致
...
```json
{{
  "synced_content": "完整内容（正文完全不变 + 已同步的分镜设计/视觉指导部分）",
  "body_changes_detected": ["检测到的正文修改点1", "修改点2"],
  "visual_updates_applied": ["已更新的分镜设计部分1", "部分2"],
...
class QualityFixGenerator:
    ...
    # v2.7: AI视觉资源已独立存储，此处仅保留分镜设计和拍摄脚本相关标记
    _VISUAL_MARKERS = [
        "拍摄脚本参考", "运镜设计", "光影方案", "演出指导", "剪辑思路", "连续性衔接",
        "分镜脚本设计", "分镜设计表", "分镜说明", "场景切换规划", "情感曲线",
        "镜头语言", "景别", "运动方式", "时长分配", "转场方式", "情感意图",
        "分镜设计方向", "视觉叙事方向", "视觉呈现建议",
        "分镜设计", "故事板", "storyboard",
    ]
    ...
    def sync_ai_resource_after_body_fix(
        self,
        has_ai_resource: bool,
        body_changes_detected: List[str],
        unit_index: int = 0
    ) -> Dict:
        """
        正文修正后，提供AI资源重新生成建议（v2.7新增）

        AI视觉资源已独立于正文存储。当正文被质控修正后，此方法
        分析是否需要通知前端建议用户重新生成AI资源。

        Args:
            has_ai_resource: 该单元是否已有AI资源内容
            body_changes_detected: 检测到的正文修改列表
            unit_index: 单元序号

        Returns:
            建议结果字典:
            - suggest_regenerate: bool — 是否建议重新生成
            - reason: str — 建议原因
            - affected_sections: List[str] — 受影响的AI资源板块
        """
        result = {
        ...
        if not has_ai_resource:
            # 没有AI资源，无需建议
            return result

        if not body_changes_detected:
            return result

        # 分析哪些正文修改可能影响AI资源
        visual_keywords = {
            "人物参考图": ["角色", "人物", "外貌", "服装", "表情", "发型", "妆容"],
            "场景参考图": ["场景", "环境", "地点", "天气", "时间", "氛围", "光线"],
            "物品参考图": ["道具", "物品", "装备", "武器", "器物"],
            "视频生成提示词": ["动作", "剧情", "打斗", "追逐", "镜头", "运镜", "分镜"],
        }

        affected = set()
        for change in body_changes_detected:
            change_lower = change.lower()
            for section, keywords in visual_keywords.items():
                if any(kw in change_lower for kw in keywords):
                    affected.add(section)

        if affected:
            result["suggest_regenerate"] = True
            result["affected_sections"] = list(affected)
            section_names = "、".join(sorted(affected))
            result["reason"] = (
                f"单元 {unit_index} 的正文修正涉及以下视觉相关内容：{section_names}。"
                f"建议重新生成AI视觉资源以确保与修正后的正文保持一致。"
            )
