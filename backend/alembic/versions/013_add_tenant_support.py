...
def upgrade():
    ...
    if 'login_count' not in users_columns:
    ...
    if 'tenant_id' not in np_columns:
    ...
    if 'tenant_id' not in kb_columns:
    ...
    if 'tenant_id' not in gen_columns:
    ...
    # API Key表
    api_columns = [col['name'] for col in inspector.get_columns('user_api_keys')]
    if 'tenant_id' not in api_columns:
